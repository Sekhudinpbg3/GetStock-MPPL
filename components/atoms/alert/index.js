    ...
    <div key={id} style={style}>
      ...
      <div className="relative w-52 p-2 bg-white dark:bg-slate-800 drop-shadow-md rounded-md">
        <div className="w-full flex justify-center dark:bg-slate-800">
          ...
          >
            {options.type === "info" ? (
              <InfoIcon fill="fill-yellow-500" />
            ) : null}
            {options.type === "success" ? (
              <SuccesIcon fill="fill-blue-500" />
            ) : null}
            {options.type === "error" ? (
              <ErrorIcon fill="fill-pink-500" />
            ) : null}
          </div>
        </div>
        <Gap heigh="h-2 dark:bg-slate-800" />
        <h1 className={`${titleStyle} text-center text-sm`}>
          {title || titleDefault}
        </h1>
        <Gap heigh="h-2 dark:bg-slate-800" />
        ...
        <div
          className={`${
            buttonHide
              ? "hidden"
              : "w-full flex justify-center items-center dark:bg-slate-800"
          }`}
        >
          <Button.Opacity
            opacity="primary"
            onClick={close}
            buttonClass="text-sm px-2 py-0.5 rounded-md font-semibold"
          >
            Ok
          </Button.Opacity>
        </div>
